...
import com.blue.blueapplication.utils.JudgeNetwork;
...
public class AsyncHttpClient {
	...
	private boolean isConectJisudai;
	 ...
	 * Creates a new AsyncHttpClient.
	 */
	public AsyncHttpClient(Context context,boolean isConectJisudai) {
		this.mContext = context;
		this.isConectJisudai = isConectJisudai;
		BasicHttpParams httpParams = new BasicHttpParams();
	...
	public void get(Context context, String url, RequestParams params,
		...
		if (JudgeNetwork.isNetWorkAvailable()) {
			...
			if (responseHandler!=null) {
				responseHandler.onFailure(null, "当前网络不可用,请检查网络连接设置");
			}
		}
	}

    public void getWithFullUrl(Context context, String fullUrl,
            AsyncHttpResponseHandler responseHandler) {
        if (JudgeNetwork.isNetWorkAvailable()) {
            sendRequest(httpClient, httpContext, new HttpGet(fullUrl), null,
	 ...
	 * @param params
	 */
	private void setUnityParams(RequestParams params) {
		//如果是极速贷不需要再这里添加公用的参数
		if (isConectJisudai) { 
			return;
		}
		...
		// params.put("RONGID",
		// MD5.md5sum(RongApplication.mobileInfo.getIMEI()));

		params.put("app_ver", "1.0");
